...
const AI_FIELD_PATTERNS = [
  { pattern: /user|customer|person/i, suggestions: ["id", "name", "email", "age", "status", "role", "createdAt"] },
  { pattern: /product|item/i, suggestions: ["id", "name", "price", "category", "stock", "sku", "inStock"] },
  { pattern: /order|transaction/i, suggestions: ["id", "status", "total", "items", "date", "customerId"] },
  { pattern: /address|location/i, suggestions: ["street", "city", "state", "zip", "country", "coordinates"] },
  { pattern: /payment|billing/i, suggestions: ["method", "status", "amount", "currency", "cardLast4"] },
  { pattern: /settings|config/i, suggestions: ["theme", "notifications", "language", "timezone"] },
]

// Smart JSON path templates
const JSON_PATH_TEMPLATES = [
  { label: "Array filter by property", path: "$.[?(@.property == 'value')]", description: "Filter array items by property value" },
  { label: "Array map property", path: "$.[*].property", description: "Get property from all array items" },
  { label: "Nested array access", path: "$.parent[*].child[*].property", description: "Access nested arrays" },
  { label: "Conditional access", path: "$..[?(@.active == true)].name", description: "Get names of active items" },
  { label: "Array slice", path: "$.[0:5]", description: "Get first 5 items from array" },
  { label: "Recursive descent", path: "$..property", description: "Find property at any depth" },
  { label: "Multiple properties", path: "$.['prop1','prop2']", description: "Select multiple properties" },
]
  ...
  if (type === "object" && data !== null) {
    Object.entries(data).forEach(([key, value]) => {
      ...
      fields.push({
        ...
        label: key,
        type: fieldType,
        value: fieldType === "object" || fieldType === "array" ? undefined : value,
        category: "field",
        preview: fieldType === "string" && value ? value.slice(0, 50) : undefined,
      })
...
export function AdvancedFieldInput({
  ...
  const [popularFields, setPopularFields] = React.useState<Record<string, number>>({})
  ...
  const aiSuggestions = React.useMemo(() => {
    ...
    if (currentPath) {
      ...
      AI_FIELD_PATTERNS.forEach(({ pattern, suggestions: fieldSuggestions }) => {
        if (pattern.test(lastPart)) {
          fieldSuggestions.forEach(suggestion => {
            const suggestedPath = [...parts.slice(0, -1), suggestion].join(".")
            suggestions.push({
              path: suggestedPath.startsWith("$") ? suggestedPath : `$.${suggestedPath}`,
              label: suggestion,
              type: "unknown",
              category: "ai",
  ...
  const allSuggestions = React.useMemo(() => {
    ...
    return suggestions
  }, [fields, extractedFields, recentFields, popularFields, aiSuggestions, allowJsonPath, search])
  ...
  const filteredSuggestions = React.useMemo(() => {
    ...
    if (search) {
      filtered = filtered
        .map(suggestion => ({
          ...suggestion,
          matchScore: fuzzyMatch(search, suggestion.path) + fuzzyMatch(search, suggestion.label),
        }))
  ...
  React.useEffect(() => {
    ...
    try {
      const pathParts = value.replace("$", "").split(/[.\[\]]/).filter(Boolean)
  ...
  const handleSelect = (suggestion: FieldSuggestion) => {
    ...
    const newRecent = [suggestion.path, ...recentFields.filter(f => f !== suggestion.path)].slice(0, MAX_RECENT_FIELDS)
  ...
  const TypeIcon = value && previewValue !== null ? (typeIcons[getFieldType(previewValue)] || FileJson) : Search
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
          ...
          {showPreview && previewValue !== null && (
            <div className="absolute right-3 top-1/2 -translate-y-1/2 z-10">
              <Badge variant="secondary" className="text-xs">
                ...
                {typeof previewValue === "string" ? previewValue.slice(0, 20) : getFieldType(previewValue)}
              </Badge>
            </div>
          )}
        </div>
      ...
      <PopoverContent className="p-0 w-[600px]" align="start" sideOffset={4}>
          ...
          <div className="max-h-[400px] overflow-y-auto p-2">
            {Object.entries(groupedSuggestions).length === 0 ? (
              ...
              Object.entries(groupedSuggestions).map(([category, suggestions]) => {
                const CategoryIcon = categoryIcons[category as keyof typeof categoryIcons] || FileJson
                ...
                return (
                  <div key={category} className="mb-4 last:mb-0">
                    ...
                    {suggestions.map((suggestion, index) => {
                      const FieldTypeIcon = typeIcons[suggestion.type as keyof typeof typeIcons] || FileJson
