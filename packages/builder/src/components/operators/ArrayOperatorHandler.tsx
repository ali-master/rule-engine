...
export const ArrayOperatorHandler: React.FC<OperatorHandlerProps> = ({
  ...
  const getOperatorDescription = () => {
    const descriptions = {
      ...
      'not-contains-any': 'Array field does NOT contain ANY of the specified values',
      'not-contains-all': 'Array field does NOT contain ALL of the specified values',
    };
    return descriptions[operator as keyof typeof descriptions] || operator;
  };

  const isArrayFieldOperator = ['contains', 'not-contains', 'contains-any', 'contains-all', 'not-contains-any', 'not-contains-all'].includes(operator);

  return (
    <div className="space-y-4">
      <div>
        <div className="flex items-center gap-2 mb-2">
          <Label>{isArrayFieldOperator ? 'Values to Check' : 'Allowed Values'}</Label>
          ...
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent className="max-w-xs">
                <p className="font-semibold mb-1">{operator.toUpperCase()} Operator</p>
        ...
        <Alert className="mb-3">
          ...
          <AlertDescription className="text-xs">
            {isArrayFieldOperator ? (
              <span>Checking if <strong>{field}</strong> (array) {getOperatorDescription().toLowerCase()}</span>
            ) : (
              <span>Checking if <strong>{field}</strong> {getOperatorDescription().toLowerCase()}</span>
            )}
          </AlertDescription>
        </Alert>
      </div>
            ...
            <div className="flex-1">
              {fieldType === 'boolean' ? (
                ...
                <Input
                  ...
                  value={item || ''}
                  onChange={(e) => updateItem(index, parseValue(e.target.value, fieldType))}
      ...
      {items.length > 0 && (
        <div className="p-3 bg-muted rounded-md">
          ...
          <code className="text-xs font-mono">
            {field} {operator} [{items.map(i => typeof i === 'string' ? `"${i}"` : i).join(', ')}]
          </code>
        </div>
      )}
    </div>
