...
import { Type, Hash, ArrowUpAZ, ArrowDownAZ, Regex, Info } from 'lucide-react';
...
export const StringValidationHandler: React.FC<OperatorHandlerProps> = ({
  ...
  const getValidationInfo = () => {
    const info = {
      'alpha': {
        ...
        examples: ['"Hello" ✓', '"Test" ✓', '"Hello123" ✗', '"Hello World" ✗ (space)'],
        needsValue: false,
      },
      'alpha-numeric': {
        title: 'Alphanumeric Only',
        description: 'String contains only letters and numbers (no spaces or special chars)',
        icon: Hash,
        color: 'text-green-600 dark:text-green-400',
        examples: ['"Hello123" ✓', '"Test456" ✓', '"Hello World" ✗', '"Hello-123" ✗'],
        needsValue: false,
      },
      'lower-case': {
      ...
      'matches': {
        ...
        examples: ['Pattern: ^[A-Z] → "Hello" ✓', 'Pattern: \\d{3}-\\d{3} → "123-456" ✓'],
        needsValue: true,
      },
      'not-matches': {
  ...
  const commonPatterns = [
    ...
    { label: 'IP Address', pattern: '^((25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\\.){3}(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$' },
  ];
    ...
    <div className="space-y-4">
      <div>
        <div className="flex items-center gap-2 mb-3">
          ...
          <Badge variant="outline" className="text-xs">
            {validationInfo.needsValue ? 'Pattern required' : 'No config needed'}
          </Badge>
        </div>
      ...
      {needsPattern && (
        <>
          ...
          {value && (
            <div className="space-y-2">
              <Label className="text-xs text-muted-foreground">Test Pattern</Label>
      ...
      <Alert>
        ...
        <AlertDescription className="text-xs">
          ...
          {value && <span className="block mt-1 font-mono">Pattern: {value}</span>}
        </AlertDescription>
      </Alert>
