...
import type { RuleType, EvaluationResult, Condition, Constraint } from '@usex/rule-engine';
...
export const RuleEvaluator: React.FC<RuleEvaluatorProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'result' | 'details' | 'data'>('result');
  ...
  const evaluateConditionDetails = useCallback((
    ...
    if (condition.and) {
      ...
      for (let i = 0; i < condition.and.length; i++) {
        ...
        if ('field' in item) {
          // It's a constraint
          const constraintResult = evaluateConstraint(item as Constraint, data);
          details.constraints?.push(constraintResult);
          if (!constraintResult.passed) allPassed = false;
        } else {
          // It's a sub-condition
          const subResult = evaluateConditionDetails(item as Condition, data, itemPath);
  ...
  const evaluateConstraint = useCallback((constraint: Constraint, data: any): {
    constraint: Constraint;
    passed: boolean;
    actualValue?: any;
    message?: string;
  ...
  const renderEvaluationDetails = (details: EvaluationDetails, depth = 0): React.ReactNode => {
    const isExpanded = expandedPaths.has(details.path);
    const hasChildren = (details.subConditions?.length || 0) > 0 || (details.constraints?.length || 0) > 0;
              ...
              {details.constraints?.map((constraint, index) => (
                ...
                >
                  ...
                  <span className="font-mono text-xs">
                    {constraint.constraint.field} {constraint.constraint.operator}{' '}
                      ...
                      >
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          <span className="text-lg font-semibold">
                            Rule {evaluationResult.isPassed ? 'Passed' : 'Failed'}
                          </span>
                        </div>
                        
                        {evaluationResult.value && (
                          <div className="mt-3">
                            <Label className="text-sm text-muted-foreground">Result Value:</Label>
                        ...
                        {evaluationResult.message && (
                          <div className="mt-3">
                            ...
                            <p className="mt-1 text-sm">{evaluationResult.message}</p>
                          </div>
                        )}
                      </div>
                    </div>
                  ...
                  <ScrollArea className="h-[400px] pr-4">
                    {evaluationDetails.length > 0 ? (
                      <div className="space-y-2">
                        {evaluationDetails.map((detail) => renderEvaluationDetails(detail))}
                      </div>
                    ) : (
                      <div className="text-center py-8 text-muted-foreground">
